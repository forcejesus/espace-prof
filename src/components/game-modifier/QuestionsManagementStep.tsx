...
import { Plus, Settings, MessageSquare, Clock, CheckCircle, HelpCircle, ChartBar, Trash2, Edit3 } from "lucide-react";
...
import { QuestionCard as ExistingQuestionCard } from "@/components/quiz-creator/components/QuestionCard";
import { questionService } from "@/services";
import { CreatedQuestion } from "@/components/quiz-creator/utils/questionTypeUtils";
...
export function QuestionsManagementStep({ game, onQuestionUpdate }: QuestionsManagementStepProps) {
  ...
  const saveLocalQuestion = async (localQuestion: any) => {
    try {
      ...
      const questionData = {
        ...
        point: "686a8704dc5ff531557af515",
  ...
  const getTypeQuestionId = (type: string) => {
    switch (type) {
      case "VRAI_FAUX":
        return "6839d9de4dd3e891de5ff98f";
      case "CHOIX_UNIQUE":
        return "6839d9de4dd3e891de5ff98e";
      case "CHOIX_MULTIPLE":
        return "6839d9de4dd3e891de5ff990";
      case "REPONSE_COURTE":
        return "6839d9de4dd3e891de5ff991";
      default:
        return "6839d9de4dd3e891de5ff98f";
    }
  };
    ...
    <Card className="shadow-sm border-orange-200">
      ...
      <CardContent className="p-6">
        ...
        {questions.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-lg font-semibold text-orange-800">Questions existantes ({questions.length})</h4>
            </div>
            <div className="space-y-4">
              {questions.map((question, index) => (
                <div key={question._id}>
                  <ExistingQuestionCard
                    question={question as CreatedQuestion}
                    index={index}
                    onQuestionUpdated={() => {
                      if (onQuestionUpdate) {
                        onQuestionUpdate();
        ...
        {/* Aperçu des questions avec réponses mises en évidence */}
        {questions.length > 0 && (
          <div className="mb-8">
            <h4 className="text-lg font-semibold text-orange-800 mb-4">
              Aperçu des réponses correctes
            </h4>
            <div className="space-y-4">
              {questions.map((question, index) => (
                <div 
                ...
                >
                  ...
                  {/* Affichage des réponses avec mise en évidence */}
        ...
        {/* Actions rapides */}
        {(questions.length > 0 || localQuestions.length > 0) && (
          <div className="flex flex-wrap items-center justify-between gap-4 bg-gradient-to-r from-white/90 to-orange-50/80 backdrop-blur-sm rounded-2xl p-6 border border-orange-200/60">
