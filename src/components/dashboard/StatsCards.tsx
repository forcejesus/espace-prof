...
export function StatsCards() {
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-s20">
      {stats.map((stat, index) => <Card key={index} className={`${stat.bgColor} border border-akili-grey-300 shadow-akili-sm hover:shadow-akili-md transition-all duration-300 rounded-xl overflow-hidden group`}>
          <CardContent className="p-s20">
            ...
            <div className="flex items-start justify-between mb-s16 mx-0 my-[25px]">
              <div className="flex items-center space-x-s12">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${stat.color === 'orange' ? 'bg-akili-orange-100' : stat.color === 'blue' ? 'bg-akili-blue-100' : stat.color === 'green' ? 'bg-akili-green-100' : 'bg-akili-purple-100'}`}>
                  <stat.icon className={`w-6 h-6 ${stat.color === 'orange' ? 'text-akili-orange-600' : stat.color === 'blue' ? 'text-akili-blue-600' : stat.color === 'green' ? 'text-akili-green-600' : 'text-akili-purple-500'}`} />
                </div>
              </div>
              <Badge className={`text-xs font-medium ${stat.trend.startsWith('+') ? 'bg-akili-green-100 text-akili-green-700' : 'bg-akili-red-100 text-akili-red-700'}`}>
